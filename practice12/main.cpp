...
const vec2 disjoint = vec2(0.0, 0.0);
vec2 intersect(vec3 ray_dir, vec3 ray_start){
    vec2 res = vec2(0.0, 10.0);
    for(int i=0; i<3;i++){
        if ((ray_start[i]<box_min[i] && ray_dir[i] <=0.0)||(ray_start[i]>box_max[i] && ray_dir[i]>=0.0)) return disjoint;
    }
    for(int i=0; i<3;i++){
        if(ray_start[i]<box_min[i]){
            if (ray_dir[i] <= 0) return disjoint;
            res.x = max(res.x, (box_min[i]-ray_start[i])/ray_dir[i]);
            res.y = min(res.y, (box_max[i]-ray_start[i])/ray_dir[i]);
        } else if (ray_start[i]<box_max[i]){
            if (ray_dir[i]<0){
                res.y = min(res.y, (box_min[i]-ray_start[i])/ray_dir[i]);
            } else if (ray_dir[i]>0){
                res.y = min(res.y, (box_max[i]-ray_start[i])/ray_dir[i]);
            }
        } else {
            if (ray_dir[i] >= 0) return disjoint;
            res.x = max(res.x, (box_max[i]-ray_start[i])/ray_dir[i]);
            res.y = min(res.y, (box_min[i]-ray_start[i])/ray_dir[i]);
        }
    }
    return res;
...
void main()
{
    vec3 camera_dir_norm = normalize(position - camera_position);
    vec2 intersection = intersect(camera_dir_norm, camera_position);

    vec3 color = vec3((intersection.y - intersection.x)/3.5);
