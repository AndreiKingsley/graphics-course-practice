...
const char debug_rectangle_vertex_shader_source[] =
R"(#version 330 core
...
void main()
{
	...
	texcoord = vertex + vec2(0.5);
}
)";

const char debug_rectangle_fragment_shader_source[] =
R"(#version 330 core
...
void main()
{
    ...
    shadow_pos = shadow_pos * vec4(0.5) + vec4(0.5);
    shadow = texture(shadow_map, shadow_pos.xy).r < shadow_pos.z;
...
int main() try {
    ...
    GLuint prog_shadow_transform_location = glGetUniformLocation(program, "shadow_transform");
    ...
    auto debug_rectangle_vertex_shader = create_shader(GL_VERTEX_SHADER, debug_rectangle_vertex_shader_source);
    auto debug_rectangle_fragment_shader = create_shader(GL_FRAGMENT_SHADER, debug_rectangle_fragment_shader_source);
    auto debug_rectangle_program = create_program(debug_rectangle_vertex_shader, debug_rectangle_fragment_shader);

    GLuint render_result_location = glGetUniformLocation(debug_rectangle_program, "render_result");
    ...
    GLuint model_shadow_location = glGetUniformLocation(shadow_program, "model");

    GLuint debug_vao;
    glGenTextures(1, &debug_vao);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, shadow_map_res, shadow_map_res, 0, GL_DEPTH_COMPONENT,
                 GL_FLOAT, nullptr);
    ...
    while (running) {
        ...
        glm::vec3 light_X = glm::vec3(-light_direction.y, light_direction.x, 0.f);
        glm::vec3 light_Y = glm::cross(light_X, light_Z);

        glm::mat4 shadow_transform = glm::mat4{
        ...
        glUniformMatrix4fv(model_shadow_location, 1, GL_FALSE, reinterpret_cast<float *>(&model));
        glBindVertexArray(vao);
        ...
        glUniformMatrix4fv(prog_shadow_transform_location, 1, GL_TRUE, reinterpret_cast<float *>(&shadow_transform));
        ...
        glUseProgram(debug_rectangle_program);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, shadow_map);
